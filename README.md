...
Configure pull secret for egress gateways namespace. Get pull secret to pull CaliEnt images and copy it into the namespace you will create egress gateways in (e.g. egress-gw)
...
If you've configured Egress gateways feature on per namespace basis, then apply annotations to namespaces only. If you enabled the feature to be use on per pod basis too, then annotate the pods that should use egress gateways functionality.

This example labels egress gateways with egress-code: red label. This key-value pair is used to annotate the namespace that hosts pods configured to use egress gateways.

Deploy namespace and the client pod (netshoot) to test the feature operation.

kubectl apply -f app/
The namespace definition in 00-namespace.yaml already includes required annotations. However, if you choose to use a different namespace, you can label the namespace following example below:
